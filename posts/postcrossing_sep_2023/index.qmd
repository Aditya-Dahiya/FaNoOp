...
subtitle: "Analyzing Postcrossing.com data reveals how postcard sending relates to GDP, literacy, and demographics."
...
top_users |>
  ...
  scale_x_discrete(labels = c(
    "Ranks - All Time (since inception)",
...
```{r}
#| label: fig-regression-1
#| fig-cap: "GDP per capita is the most important predictor of average number of postcards sent by each member"
#| results: asis
...
Thus, the most important correlation of Average number of postcards sent by a member is with wealth, the GDP per capita. This relation is depicted below in @fig-reg1 .

```{r}
#| label: fig-reg1
#| fig-cap: "Average number of postcards sent correlates with the wealth of the country, i.e., GDP per capita"
#| fig-cap-location: "top"

df_reg |>
  filter(members >= 100) |>
  ggplot(aes(x = ny_gdp_pcap_pp_cd,
             y = postcard_per_mem,
             label = country_name)) +
  geom_smooth(lwd = 2, 
              col = "grey", alpha = 0.3,
              method = "lm") +
  geom_point() +
  ggrepel::geom_text_repel(max.iter = 100,
                           size = 3) +
  scale_y_continuous() + 
  scale_x_continuous(labels = scales::label_number_si()) +
  theme_minimal() +
  ...
  labs(x = "GDP per capita (current US$)",
       y = "Average Number of postcards sent by each member")
```

Now, lets try to see what could explain the different number of members in different countries, i.e. Percentage of the population that is registered on [postcrossing.com](www.postcrossing.com). We see in the table below that the most important predictor of postcrossing member's proportion in a population is the share of elderly population. Also, we see the fast growing populations have lower postcrossing members.
...
#| fig-cap: "Regressions results indicate importance of elderly population and population growth rate in predicting the number of members who use postcrossing"
#| results: asis
...
df_reg |>
  ...
  ggplot(aes(x = members,
             y = postcard_per_mem,
             label = country_name)) +
  geom_smooth(se = FALSE, lwd = 2, col = "grey", alpha = 0.7,
              method = "lm") +
  geom_point() +
  ggrepel::geom_text_repel(max.iter = 100,
                           size = 3) +
  ...
  theme_minimal() +
  theme(panel.grid.minor = element_blank()) +
  labs(x = "Number of postcrossing members (on log scale)",
       y = "Average Number of postcards sent by each member")
```
...
df_reg |>
  ...
  theme_minimal() +
  theme(panel.grid.minor = element_blank()) +
  labs(x = "Percentage of population using postcrossing",
       y = "Average Number of postcards sent by each member")
```

## The bottom line: --

1.  Postcrossing is most popular in nordic and eastern European countries, and catching up in USA and China.

2.  Countries with higher percentage of elderly people also have higher percentage of population registered on postcrossing.

3.  Being elderly or young has no correlation with the number of postcards sent. Instead, it is wealth (GDP per capita) that correlates with the number of postcards sent on average by a member, in a country.

### Happy postcrossing on the World Postcards Day - October 1!

![](Postcrossing%20Main%20Logo.png){width="427"}
